...
import { setValueQ, setValueE } from '../service/store';

export default {
  data() {
    return {
      isSidebarHide: false,
      mainScrollableContent: null as HTMLElement | null,
      visible: false,
    };
  },
  mounted() {
    this.mainScrollableContent = this.$refs.mainScrollableContent as HTMLElement;
  },
  methods: {
    hideSidebar() {
    ...
    scrollWithOffset(offset: number) {
      const currentScrollTop = this.mainScrollableContent?.scrollTop || 0;
      const targetScrollTop = currentScrollTop + offset;

      this.animateScroll(currentScrollTop, targetScrollTop, 500);
    },
    animateScroll(start: number, end: number, duration: number) {
  ...
  setup() {
    ...
    onMounted(() => {
      mainScrollableContent.value = quasar.$refs.mainScrollableContent as HTMLElement;
    });
